...
api.history.init = function() {
  var a = navigator.userAgent.toLowerCase(), b = -1 != a.indexOf("safari") && -1 == a.indexOf("chrome"), c = !1;
  window.onpopstate = function(a) {
    b && !c ? c = !0 : api.page.load(location.href, !1);
  };
};
api.links = {};
api.links.typeLinkClickWithScroll = function(a) {
  return a.ctrlKey || a.metaKey ? !0 : api.page.load($(this).attr("href"), !0, !0);
};
...
api.search.match_ = function(a, b) {
  return -1 != a.toLowerCase().indexOf(b.toLowerCase());
};
api.search.findConstants_ = function(a) {
  for (var b = [], c = api.search.data_, e = 0;e < c.namespaces.length;e++) {
    for (var d = c.namespaces[e], f = 0;f < d.constants.length;f++) {
      var h = d.constants[f];
      api.search.match_(h.name, a) && b.push(h);
    }
  }
  return b;
};
api.search.findGrouped_ = function(a, b, c) {
  for (var e = api.search.data_, d = api.search.match_, f = [], h = 0;h < e[b].length;h++) {
    for (var k = e[b][h], g = 0;g < k[c].length;g++) {
      var l = k[c][g];
      d(l.name, a) && f.push(l);
    }
  }
  ...
  for (h = 0;h < f.length;h++) {
    ...
    for (g = 0;g < a.length;g++) {
      a[g].name == b.name && (c = !0, a[g].multiple = !0, a[g].group ? a[g].group.push(b) : a[g].group = [a[g], b]);
    }
    c || a.push($.extend({}, b));
  }
  return a;
};
api.search.filter_ = function(a, b, c) {
  c = [];
  for (var e = 0;e < b.length;e++) {
    var d = b[e];
    api.search.match_(d.name, a) && c.push(d);
  }
  return c;
};
api.search.findMethods_ = function(a) {
  return api.search.findGrouped_(a, "classes", "methods");
};
api.search.findFunctions_ = function(a) {
  return api.search.findGrouped_(a, "namespaces", "functions");
};
api.search.findNamespaces_ = function(a) {
  return api.search.filter_(a, api.search.data_.namespaces, "namespace");
};
api.search.findClasses_ = function(a) {
  return api.search.filter_(a, api.search.data_.classes, "class");
};
api.search.findEnums_ = function(a) {
  return api.search.filter_(a, api.search.data_.enums, "enum");
};
api.search.findTypedefs_ = function(a) {
  return api.search.filter_(a, api.search.data_.typedefs, "typedef");
};
api.search.showGrouped_ = function(a, b, c) {
  api.breadcrumb.showSearch(a.name);
  $("#search-results-new").hide();
  for (var e = $("<ul></ul>"), d = 0;d < a.group.length;d++) {
    var f = a.group[d];
    e.append("<li><a class='item-link' href='/" + api.config.version + "/" + f.link + "'>" + b + f["full-name"] + c + "</a></li>");
  }
  api.page.showSearchResults(e);
};
api.search.addToResults_ = function(a, b, c, e, d) {
  if (b.length) {
    a.append("<li class='group-name'>" + d + "</li>");
    for (d = 0;d < b.length;d++) {
      var f = b[d];
      f.multiple && function(b) {
        var d = $("<li><a class='group-link' href='#'>" + c + b.name + e + " <span>" + b.group.length + " matches</span></a></li>");
        d.find("a").click(function() {
          api.search.showGrouped_(b, c, e);
          return !1;
        });
        a.append(d);
      }(f);
    ...
    for (d = 0;d < b.length;d++) {
      f = b[d], f.multiple || a.append("<li><a class='item-link' href='/" + api.config.version + "/" + f.link + "'>" + c + f["full-name"] + e + "</a></li>");
    }
  }
};
api.search.showEmpty_ = function(a) {
  a.append("<li>No results found</li>");
};
api.search.search_ = function(a) {
  if (3 > a.length) {
    api.search.hide();
  } else {
    var b = $("<ul></ul>");
    api.search.addToResults_(b, api.search.findConstants_(a), "", "", "Constants");
    api.search.addToResults_(b, api.search.findFunctions_(a), "", "()", "Functions");
    api.search.addToResults_(b, api.search.findMethods_(a), "", "()", "Methods");
    api.search.addToResults_(b, api.search.findNamespaces_(a), "", "", "Namespaces");
    api.search.addToResults_(b, api.search.findEnums_(a), "[", "]", "Enums");
    api.search.addToResults_(b, api.search.findTypedefs_(a), "{", "}", "Typedefs");
    api.search.addToResults_(b, api.search.findClasses_(a), "", "", "Classes");
    b.find("a.item-link").click(api.links.typeLinkClick);
    b.find("a").length || api.search.showEmpty_(b);
    $("#search-results-new").show();
    $("#search-results-new").html("");
    $("#search-results-new").append(b);
  }
};
api.search.onLoad_ = function(a) {
  api.search.data_ = a;
  $("#search-results-new").click(function(a) {
    a.stopPropagation();
  });
  ...
  $("#search").keyup(function(a) {
    27 == a.keyCode ? (api.search.hide(), $("#search").val("")) : api.search.search_($(this).val());
  });
};
...
api.resize.hideSidebar_ = function(a) {
  api.resize.sidebarVisible_ = !1;
  var b = api.resize.hiddenMenuSize_;
  a ? ($("#menu-bar").animate({width:b}, 300), $("#content-wrapper").animate({marginLeft:b, width:window.innerWidth - b}, 300), $("#search-form").animate({opacity:0}, 200), $("#tree-wrapper").animate({opacity:0}, 200), $(".breadcrumb").animate({left:b}, 300), $("#footer").animate({opacity:0}, 200)) : ($("#menu-bar").css("width", b), $("#content-wrapper").css("width", window.innerWidth - b).css("margin-left", b), $("#search-form").css("opacity", 0), $("#tree-wrapper").css("opacity", 0), $("#footer").css("opacity", 
  0), $(".breadcrumb").css("left", b));
  $("a.switcher .fa").attr("class", "fa fa-chevron-right");
  $("#size-controller").css("cursor", "default");
  api.tree.disableScrolling();
};
api.resize.showSidebar_ = function(a) {
  api.resize.sidebarVisible_ = !0;
  var b = api.resize.visibleMenuSize_;
  a ? ($("#menu-bar").animate({width:b}, b), $("#content-wrapper").animate({marginLeft:b, width:window.innerWidth - b}, b), $(".breadcrumb").animate({left:b}, b)) : ($("#menu-bar").css("width", b), $("#content-wrapper").css("width", window.innerWidth - b).css("margin-left", b), $(".breadcrumb").css("left", b));
...
api.resize.toggleSidebar_ = function() {
  api.resize.sidebarVisible_ ? api.resize.hideSidebar_(!0) : api.resize.showSidebar_(!0);
};
api.resize.contentSize_ = function() {
  992 > window.innerWidth ? api.resize.hideSidebar_(!1) : (api.resize.showSidebar_(!1), $("#content-wrapper").css("width", window.innerWidth - 300).css("margin-left", 300), $(".breadcrumb").css("left", $("#menu-bar").width()));
...
api.breadcrumb.update = function(a) {
  ...
  for (var b = 0;b < a.length;b++) {
    var c;
    b < a.length - 1 ? (c = a.slice(0, b + 1).join("."), c = $("<li><a href='/" + api.config.version + "/" + c + "'>" + a[b] + "</a></li>"), c.find("a").click(api.links.typeLinkClickWithScroll)) : c = $("<li class='active'>" + a[b] + "</li>");
    $("ol.breadcrumb").append(c);
  }
};
api.breadcrumb.showSearch = function(a) {
  $("ol.breadcrumb li").remove();
  $("ol.breadcrumb").append("<li class='active'>Search results for " + a + "</li>");
};
api.tree = {};
...
api.tree.disableScrolling = function() {
  $("#tree-wrapper").mCustomScrollbar("disable", !0);
};
...
api.pageScrolling.onContentScroll_ = function() {
  ...
  a && a != api.pageScrolling.currentVisible_ && (api.pageScrolling.currentVisible_ = a, api.tree.expand(api.config.page, a), api.page.highlight(a, !1, !1));
};
...
api.pageScrolling.highlightScroll = function(a) {
  setTimeout(function() {
...
api.page.highlight = function(a, b, c) {
  ...
  b && (b = api.utils.getEntryFromUrl(location.pathname), api.tree.expand(b, a));
  c && api.pageScrolling.highlightScroll(taget);
...
api.page.load = function(a, b, c) {
  api.search.hide();
  void 0 == b && (b = !0);
  0 == a.indexOf("#") && (a = location.pathname + a);
  var e = api.utils.cleanupPath(a), d;
  -1 != a.indexOf("#") && (d = a.substr(a.indexOf("#") + 1));
  if (e == location.pathname) {
    return d && api.page.highlight(a.substr(a.indexOf("#") + 1)), !1;
  }
  if ("undefined" == typeof window.history) {
    return !0;
  }
  b && window.history.pushState(null, null, a);
  api.tree.expand(a);
  c && api.tree.scrollToEntry(e, d);
  ...
  $.get(e + "/data", function(a) {
    document.title = a.title;
    $("#content-wrapper").html('<div id="content"><div class="content-container">' + a.content + "</div></div>");
    api.config.page = a.page;
    api.pageScrolling.update();
    $("#warning a").attr("href", "/" + $("#warning a").attr("data-last-version") + "/try/" + a.page);
    api.breadcrumb.update(a.page);
    api.page.fixLinks();
    api.page.fixListings();
    d && api.page.highlight(d);
  });
  return !1;
};
api.page.showSearchResults = function(a) {
  a.find("a").click(api.links.typeLinkClick);
  api.pageScrolling.destroy();
  $("#content-wrapper").html('<div id="content"><div class="content-container"></div></div>');
  $("#content .content-container").append(a);
  api.pageScrolling.update();
};
api.versions = {};
...
api.core = {};
api.core.init = function(a, b) {
  api.config.version = a;
  api.config.page = b;
  api.versions.init();
  api.resize.init();
  api.page.fixLinks();
  ...
  api.tree.scrollToEntry(b, location.hash ? location.hash.substr(1) : null);
